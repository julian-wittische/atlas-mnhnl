...
tribe_counts <- DB_taxo %>%
  ...
  mutate(
    Tribe = factor(Tribe, levels = Tribe), prop = n / sum(n),label = paste0(Tribe, "\n(", n, ")"), ypos = cumsum(n) - 0.5 * n   
  )
...
tribe_palette <- tribe_counts %>%
  ...
  group_map(~ setNames(
    colorRampPalette(c(lighten(base_colors[[.y$Subfamily]], 0.5),
                       darken(base_colors[[.y$Subfamily]], 0.3)))(nrow(.x)),as.character(.x$Tribe)
  )) %>%
  unlist()
...
ggplot(tribe_counts, aes(x = "", y = n, fill = Tribe)) +
  ...
  geom_text(aes(y = ypos, label = ifelse(prop > 0.04, label, "")),
            color = "white", size = 3.5, fontface = "bold") +
  geom_text_repel(
    data = subset(tribe_counts, prop <= 0.04), aes(y = ypos, label = label),
    size = 3.5, nudge_x = 0.6,  segment.color = NA, show.legend = FALSE )
...
ggplot(Subfamily_counts, aes(x = "", y = n, fill = Subfamily)) +
  ...
  theme_void() +
  theme(legend.position = "right", legend.text = element_text(size = 13), legend.title = element_text(size = 15)) +
  geom_text(aes(y = ypos, label = ifelse(prop > 0.03, label, "")),color = "white", size = 3.5, fontface = "bold") +
  
  geom_text_repel(
    data = subset(Subfamily_counts, prop <= 0.03),
    aes(y = ypos, label = label),size = 3.5,
    nudge_x = 0.6,   segment.color = NA,    show.legend = FALSE
  )
